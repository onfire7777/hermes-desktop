...
async function ensureLocalGatewayForOffice(): Promise<void> {
  ...
  if (!isGatewayRunning()) {
    try {
      ...
      console.error("[GATEWAY] Failed to start for Office:", err);
      throw new Error(
        `Failed to start Hermes gateway: ${(err as Error).message}`,
      );
    }
  }
  ...
  throw new Error("Hermes API gateway did not become ready on port 8642.");
}
...
function setupIPC(): void {
  ...
  ipcMain.handle("sync-session-cache", (_event, limit?: number) => {
    const conn = getConnectionConfig();
    const defaultLimit = 50;
    if (conn.mode === "ssh" && conn.ssh) {
      return sshListCachedSessions(conn.ssh, limit ?? defaultLimit);
    }
    return syncSessionCache(limit ?? defaultLimit);
  });
  ipcMain.handle(
  ...
  ipcMain.handle("claw3d-start-all", async () => {
    await ensureLocalGatewayForOffice();
    return startClaw3dAll();
  });
